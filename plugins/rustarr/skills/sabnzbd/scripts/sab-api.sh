...
# LAST-RESORT direct-API path. Prefer the rustarr MCP tool (`mcp__rustarr__sabnzbd`)
# or the `rustarr sabnzbd` CLI when available (see SKILL.md). Creds come from
# rustarr's materialized env (~/.rustarr/.env, written by `rustarr setup
# plugin-hook`), then the legacy arrs config.env. RUSTARR_SABNZBD_* are aliases.
...
SABNZBD_URL="${SABNZBD_URL:-${RUSTARR_SABNZBD_URL:-}}"
SABNZBD_API_KEY="${SABNZBD_API_KEY:-${RUSTARR_SABNZBD_API_KEY:-}}"

: "${SABNZBD_URL:?set RUSTARR_SABNZBD_URL (rustarr plugin settings) or SABNZBD_URL}"
: "${SABNZBD_API_KEY:?set RUSTARR_SABNZBD_API_KEY (rustarr plugin settings) or SABNZBD_API_KEY}"
