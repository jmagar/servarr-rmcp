...
# LAST-RESORT direct-API path. Prefer the rustarr MCP tool (`mcp__rustarr__prowlarr`)
# or the `rustarr prowlarr` CLI when available (see SKILL.md). Creds come from
# rustarr's materialized env (~/.rustarr/.env, written by `rustarr setup
# plugin-hook`), then the legacy arrs config.env. RUSTARR_PROWLARR_* are aliases.
...
PROWLARR_URL="${PROWLARR_URL:-${RUSTARR_PROWLARR_URL:-}}"
PROWLARR_API_KEY="${PROWLARR_API_KEY:-${RUSTARR_PROWLARR_API_KEY:-}}"

: "${PROWLARR_URL:?set RUSTARR_PROWLARR_URL (rustarr plugin settings) or PROWLARR_URL}"
: "${PROWLARR_API_KEY:?set RUSTARR_PROWLARR_API_KEY (rustarr plugin settings) or PROWLARR_API_KEY}"
