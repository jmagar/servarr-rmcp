...
# Radarr API wrapper — LAST-RESORT direct-API path.
#
# Prefer the rustarr MCP tool (`mcp__rustarr__radarr`) or the `rustarr radarr`
...
# legacy arrs config.env. RUSTARR_RADARR_* names are accepted as aliases.
...
RADARR_URL="${RADARR_URL:-${RUSTARR_RADARR_URL:-}}"
RADARR_API_KEY="${RADARR_API_KEY:-${RUSTARR_RADARR_API_KEY:-}}"

: "${RADARR_URL:?set RUSTARR_RADARR_URL (rustarr plugin settings) or RADARR_URL}"
: "${RADARR_API_KEY:?set RUSTARR_RADARR_API_KEY (rustarr plugin settings) or RADARR_API_KEY}"
